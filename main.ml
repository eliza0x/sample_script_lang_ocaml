...
let () =
    let string_from_tokens t = 
        List.fold_right ~f:(fun a b -> Lexer.show_token a ^ " " ^ b) ~init:"" t
    in let tokens = Lexer.tokenize "( 1 + 2 ) + 3"
    in print_string (show_expr (Option.value (parse tokens) ~default:(Num 0)) ^ "\n")
